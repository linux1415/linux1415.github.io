...
<html lang="en">
  <head>
	...
	function add_resource(a) {
		var id = a;
		window.location.href = "add_resource.php?id=" + id;
	}
	</script>
	<style>
	th {
  ...
  <body>
	...
	<nav class="navbar sticky-top navbar-expand-lg navbar-dark bg-dark">
	  ...
	  <div class="collapse navbar-collapse order-1 dual-collapse2" id="navbarNavAltMarkup">
		<div class="navbar-nav">
		  ...
		  <a class="nav-item nav-link active" href="edit_resources.php">Manage</a>
		</div>
	  </div>
	...
	<br>
	<?
	$query = "select resource as category, id from resource_categories where id in (select category_id from edit_categories_permissions where user_id = '$user') order by category;";
	...
	while ($row = mysqli_fetch_array($array, MYSQLI_ASSOC)){
		...
		echo '</b></div>';
		  echo '<div class="card-body">';
				echo "<button type='button' class='btn bg-dark' onclick='add_resource(\"$id\")' style='color:white;width:9em;font-size:14pt;'>Add Resource</button>&nbsp;<br class='hidden-ss'>";
